  ...
  tryCatch({
    ...
    for(var in getMetadata_headers){
      ...
      if(var %in% cat_vars){
        ...
        pca_plot <- autoplot(prcomp(t(counts_sorted)), data = metadata_sorted, colour = var)
        ggsave(file=paste('temp_', var, '.png', sep=""), h=6, w=6, units="in",dpi=300)
      }else if(var %in% num_vars){
        pca_plot <- autoplot(prcomp(t(counts_sorted)), data = metadata_sorted, colour = var) + scale_color_gradientn(colours = terrain.colors(7))
        ggsave(file=paste('temp_', var, '.png', sep=""), h=6, w=6, units="in",dpi=300)
      }
    }
    
    write.csv(pca_matrix$x, file="pca_matrix_output.csv")
    write.csv(metadata_sorted, file="metadata_sorted_output.csv")
...
model_pcs <- function(pca_matrix, metadata_file, output_name, cat_vars, num_vars, all_vars){
  tryCatch({
    ...
    print(pvalues)
    colnames(pvalues) <- unlist(lapply(seq(1,length(all_vars)),function(x) paste(c('PC',x),collapse='')))
    print(pvalues)
    
    
    # get the row and column names matching the adj R-sq values
    rownames(adjRsq) <- all_vars
    colnames(adjRsq) <- unlist(lapply(seq(1,length(all_vars)),function(x) paste(c('PC',x),collapse='')))
...
##input:  None -- prompts user for location of counts_file and metadata_file  ##
##        (properly formatted)                                                ##
##output: If count_file path is verified as correct, proceeds to function     ##  
##        get_metadata(); will prompt user for path to metadata file; if      ## 
##        verified as correct, proceeds to PCA; else at any stage it is not   ##
##        verified then prompts user to enter path again (recursive loop      ##
##        until path is verified) unless user quits program                   ##
################################################################################ 
get_user_input <- function(){
  ...
  if ((toupper(verification_1) == "YES") | (toupper(verification_1) == "Y")){
    ...
    if (toupper(unlist(output[2]))=="YES" | toupper(unlist(output[2]))=='Y'){
      return(output_file(counts_file = count_file, metadata_file=unlist(output[1])))
    }else if ((toupper(unlist(output[2])) == "NO") | (toupper(unlist(output[2])) == 'N')){
      return(output <- get_metadata())
    }else{
